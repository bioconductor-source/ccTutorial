...
%\VignetteDepends{Ringo, biomaRt, topGO, xtable}
...
\Rfunction{install} function from the
BiocManager package.
...
<<imageRG, results=hide,echo=TRUE>>=
...
%
<<getGenesGOAnnotation, cache=TRUE>>=
ensembl  <- useMart("ensembl", dataset="mmusculus_gene_ensembl")
...
%
<<mappingGenesToProbes, cache=TRUE, results=hide>>=
...
print(sepRegRes)
@ 
<<printSepRegRes, echo=FALSE, results=tex>>=
...
%
<<processBarerraExpressionData, cache=TRUE, keep.source=TRUE, results=hide>>=
library("affy")
...
<<mapEnsToAffy, keep.source=TRUE, cache=TRUE, results=hide>>=
ensembl <- useMart("ensembl", dataset="mmusculus_gene_ensembl")
