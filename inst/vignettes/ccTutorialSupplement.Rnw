...
%\VignetteDepends{weaver, Ringo, biomaRt, topGO, xtable}
...
@ 
<<loadExpressionData, eval=FALSE, echo=FALSE>>=
...
<<mapEnsToAffy, keep.source=TRUE, eval=TRUE, cache=TRUE>>=
ensembl <- useMart("ensembl", dataset="mmusculus_gene_ensembl")
...
@ 
% alternatively:
<<loadArrayGenes, eval=FALSE, echo=FALSE>>=
