...
\author{Joern Toedling\footnote{corresponding author,%
e-mail: toedling@ebi.ac.uk}\,,
Wolfgang Huber\\[0.5cm] 
{\small EMBL European Bioinformatics Institute,
Wellcome Trust Genome Campus},\\[0.01cm]
{\small Hinxton CB10 1SD, United Kingdom}}
\date{}
\maketitle
}
...
supply custom functions which are designed towards the analysis at
hand.
...
Bioconductor (\emph{e.~g.},
\cite{Gentleman2008,biocbook2005, biocbook2008})
...
and the Cy5 channel holds the immunoprecipiate
...
%
<<posToProbeAnno, echo=FALSE, cache=TRUE, results=hide>>=
...
@ 
<<loadProbeAnno, eval=FALSE>>=
data("probeAnno")
allChrs <- chromosomeNames(probeAnno)
@ 
% TO DO: store probeAnno in package ccTutorial, remove previous chunk 
...
the start of the \textsl{Actc1} gene, which encodes the cardiac actin
protein.
%
<<chipAlongChromActc1, fig=TRUE, width=8, height=4, along=TRUE, results=hide>>=
...
these visualisations to assess the agreement between replicates.
...
%
<<smoothAlongChromActc1, fig=TRUE, width=8, height=4, along=TRUE, results=hide>>=
chipAlongChrom(X, chrom="2", xlim=c(113.8725e6,113.8835e6), ylim=c(-3,5),
               probeAnno=probeAnno, gff=mm9genes, paletteName='Set2')
...
Below, we describe one possible approach, but before we
%we have found useful, but before we want to
...
reporters.
One wants to avoid making calls supported only by few reporters.\\
...
smoothed level.
%
<<plotCher1, fig=TRUE, width=8, height=4, along=TRUE, results=hide>>=
...
%
<<H3K4me3VsExpression, fig=TRUE, include=FALSE, width=9, height=7, boxplot=TRUE, results=hide>>=
