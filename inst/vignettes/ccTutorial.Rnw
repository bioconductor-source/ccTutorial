...
%\VignetteDepends{weaver, ccTutorial, Ringo, biomaRt, topGO, xtable}
...
{\scriptsize $^\star$Email: toedling@ebi.ac.uk\hfill}}
\date{}
\maketitle
}
...
protein.
%
<<chipAlongChromActc1, eval=TRUE, fig=TRUE, include=FALSE, width=8, height=4, along=TRUE, results=hide>>=
...
%
<<smoothAlongChromActc1, eval=TRUE, fig=TRUE, include=FALSE, width=8, height=4, along=TRUE, results=hide>>=
chipAlongChrom(X, chrom="2", xlim=c(113.8725e6,113.8835e6), ylim=c(-3,5),
               probeAnno=probeAnno, gff=mm9genes, paletteName='Set2')
...
@
%
<<histogramsSmoothed, eval=TRUE, fig=TRUE, include=FALSE, width=7, height=7>>=
...
<<cherFinding, eval=TRUE, results=hide, cache=TRUE>>=
...
%
<<relateChers, eval=TRUE, results=hide>>=
chersX <- relateChers(chersX, mm9genes, upstream=5000)
@
%
<<loadCherFinding, eval=FALSE, echo=FALSE>>=
...
smoothed level.
%
<<plotCher1, eval=TRUE, fig=TRUE, include=FALSE, width=8, height=4, along=TRUE, results=hide>>=
...
%
<<H3K4me3VsExpression, eval=TRUE, fig=TRUE, include=FALSE, width=9, height=7, boxplot=FALSE, results=hide>>=
